	    ...
	    */
	    ...
	    this.go = function (selection) {
	        ...
	        if (_this.attr.tooltip === true) {
	            _this.attr.tooltip = d3.select("body").append("div").attr("class", "quorra-tooltip").attr("id", _this.attr.id + "-tooltip").style("position", "fixed").style("visibility", "hidden").style("z-index", 999);
	        }

	        // create svg and relevant plot areas
	        _this.attr.svg = _this.attr.svg.attr("id", _this.attr.id).attr("class", _this.attr.class).attr("width", width).attr("height", height);

	        _this.defs = _this.attr.svg.append('defs');

	        // set up clip path for windowing display
	        _this.defs.append("clipPath").attr("id", "clip-" + _this.attr.id).append("rect").attr("x", 0).attr("y", 0).attr("width", _this.innerwidth).attr("height", _this.innerheight);

	        // segment sections of canvas (makes axis rendering easier)
	        _this.plotregion = _this.attr.svg.append('g').attr("class", "plotregion").attr("transform", "translate(" + _this.attr.margin.left + "," + _this.attr.margin.top + ")");
	        ...
	        if (typeof _this.data[0].x === 'string') {
	            ...
	            _this.domain = [_.min(_.map(_this.data, _this.attr.x)), _.max(_.map(_this.data, _this.attr.x))];
	        ...
	        if (typeof _this.data[0].y === 'string') {
	            ...
	            if (_this.attr.layout === 'stacked') {
	                ...
	                max = _.max(_.map(ux, function (d) {
	                    var nd = _.filter(_this.data, function (g) {
	                        return _this.attr.x(g) == d;
	                    });
	                    var tmap = _.map(nd, function (g) {
	                        return _this.attr.y(g);
	                    });
	                    return _.reduce(tmap, function (a, b) {
	                        return a + b;
	                    }, 0);
	                }));
	            } else {
	            ...
	            _this.range = [_.min(_.map(_this.data, _this.attr.y)), max];
	    ...
	    this.redraw = function (xrange, yrange, cache) {
	        ...
	        _this.axes();
	        _this.plotarea = _this.plotregion.append('g').attr('class', 'plotarea').attr('clip-path', 'url(#clip-' + _this.attr.id + ')');
	        _this.plot();
	    ...
	    this.axes = function () {
	        ...
	        // x scale formatting
	        _this.xmapper = function (d) {
	            return d;
	        };
	        if (typeof _this.data[0].x === 'string') {
	            _this.xord = d3.scale.ordinal().domain(_this.domain).range(_.range(_this.domain.length));
	            _this.xordrev = d3.scale.ordinal().domain(_this.xord.range()).range(_this.xord.domain());
	            _this.xmapper = function (d) {
	                return _this.xord(d);
	            };
	        ...
	        _this.xscale = d3.scale.linear().domain([domain[0], domain[domain.length - 1]]).range([0, _this.innerwidth]);
	        ...
	        if (typeof _this.data[0].x === 'string') {
	            ...
	            _this.xaxis = _this.xaxis.tickValues(_.range(Math.ceil(domain[0]), Math.floor(domain[domain.length - 1]) + 1)).tickFormat(function (d) {
	                return _this.attr.xformat(_this.xordrev(d));
	            });
	        }

	        // y scale formatting
	        _this.ymapper = function (d) {
	            return d;
	        };
	        if (typeof _this.data[0].y === 'string') {
	            _this.yord = d3.scale.ordinal().domain(_this.range).range(_.range(_this.range.length));
	            _this.yordrev = d3.scale.ordinal().domain(_this.yord.range()).range(_this.yord.domain());
	            _this.ymapper = function (d) {
	                return _this.yord(d);
	            };
	        ...
	        _this.yscale = d3.scale.linear().domain([range[0], range[range.length - 1]]).range([_this.innerheight, 0]);
	        ...
	        if (typeof _this.data[0].y === 'string') {
	            ...
	            _this.yaxis = _this.yaxis.tickValues(_.range(Math.ceil(range[0]), Math.floor(range[range.length - 1]) + 1)).tickFormat(function (d) {
	        ...
	        if (_this.attr.xaxis !== "hidden" && _this.attr.xaxis !== false) {
	            _this.plotregion.append("g").attr("class", "x axis").attr("transform", "translate(0," + _this.innerheight + ")").call(_this.xaxis).append("text").attr("class", "label").attr("x", function (x) {
	            ...
	            }).style("text-anchor", _this.attr.labelposition).text(_this.attr.xlabel);
	        }

	        // y axis
	        if (_this.attr.yaxis !== "hidden" && _this.attr.yaxis !== false) {
	            _this.plotregion.append("g").attr("class", "y axis").call(_this.yaxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("x", function (x) {
	                if (_this.attr.labelposition === "end") {
	                    return 0;
	                } else if (_this.attr.labelposition === "middle") {
	            ...
	            }).attr("dy", ".71em").style("text-anchor", _this.attr.labelposition).text(_this.attr.ylabel);
	        }
	    };
	    ...
	    this.slider = function () {
	        ...
	        // scaling methods
	        var updatePlot = function updatePlot(outer, cache) {
	            ...
	            outer.redraw([x1, x2], [y1, y2], cache);
	        };

	        var updateSlider = function updateSlider(outer) {
	            var obj = outer.attr.slider.__parent__;
	    ...
	    this.drawlegend = function () {
	        ...
	        var leg = _this.attr.svg.append("g").attr("transform", "translate(" + (width - _this.attr.margin.right) + "," + _this.attr.margin.top + ")").selectAll(".legend").data(data).enter().append("g").attr("class", "legend").attr("id", _this.attr.id + "-legend").attr("transform", function (d, i) {
	            return "translate(0," + (_this.attr.lmargin.top + i * 20) + ")";
	        });

	        // draw group shape
	        var selector;
	        if (_this.attr.lshape === "square") {
	            selector = leg.append("rect").attr("class", "selector").attr("x", 5 + _this.attr.lmargin.left).attr("y", _this.attr.lmargin.top).attr("width", 18).attr("height", 18).attr("rx", 5).attr("ry", 5).style("fill", _this.pallette).style("fill-opacity", function (d) {
	                return _.contains(_this.attr.toggled, d) ? 0 : _this.attr.opacity;
	            });
	        } else if (_this.attr.lshape === "circle") {
	            selector = leg.append("circle").attr("class", "selector").attr("cx", 20 + _this.attr.lmargin.left).attr("cy", 8 + _this.attr.lmargin.top).attr("r", 9).style("fill", _this.pallette).style("fill-opacity", function (d) {
	                return _.contains(_this.attr.toggled, d) ? 0 : _this.attr.opacity;
	            });
	        }
	        ...
	        leg.append("text").attr("x", function () {
	        ...
	        }).attr("y", 9 + _this.attr.lmargin.top).attr("dy", ".35em").style("text-anchor", function () {
	            if (_this.attr.lposition === "inside") {
	                return "end";
	            } else if (_this.attr.lposition === "outside") {
	                return "beginning";
	    ...
	    this.drawglyphs = function () {
	        ...
	        // refresh glyph
	        _this.defs.append('symbol').attr('id', 'glyph-refresh').attr('viewBox', '0 0 1024 1024').html(['<path d="M134.32 166.32c93.608-102.216 228.154-166.32 377.68-166.32 282.77 0 512 229.23 512 512h-96c0-229.75-186.25-416-416-416-123.020 0-233.542 53.418-309.696 138.306l149.696 149.694h-352v-352l134.32 134.32z"></path>', '<path d="M96 512c0 229.75 186.25 416 416 416 123.020 0 233.542-53.418 309.694-138.306l-149.694-149.694h352v352l-134.32-134.32c-93.608 102.216-228.154 166.32-377.68 166.32-282.77 0-512-229.23-512-512h96z"></path>'].join(''));

	        // zoom glyph
	        _this.defs.append('symbol').attr('id', 'glyph-search').attr('viewBox', '0 0 1024 1024').append('path').attr('d', 'M992.262 871.396l-242.552-206.294c-25.074-22.566-51.89-32.926-73.552-31.926 57.256-67.068 91.842-154.078 91.842-249.176 0-212.078-171.922-384-384-384-212.076 0-384 171.922-384 384s171.922 384 384 384c95.098 0 182.108-34.586 249.176-91.844-1 21.662 9.36 48.478 31.926 73.552l206.294 242.552c35.322 39.246 93.022 42.554 128.22 7.356s31.892-92.898-7.354-128.22zM384 640c-141.384 0-256-114.616-256-256s114.616-256 256-256 256 114.616 256 256-114.614 256-256 256z');

	        // image glyph
	        _this.defs.append('symbol').attr('id', 'glyph-image').attr('viewBox', '0 0 1024 1024').html(['<path d="M959.884 128c0.040 0.034 0.082 0.076 0.116 0.116v767.77c-0.034 0.040-0.076 0.082-0.116 0.116h-895.77c-0.040-0.034-0.082-0.076-0.114-0.116v-767.772c0.034-0.040 0.076-0.082 0.114-0.114h895.77zM960 64h-896c-35.2 0-64 28.8-64 64v768c0 35.2 28.8 64 64 64h896c35.2 0 64-28.8 64-64v-768c0-35.2-28.8-64-64-64v0z"></path>', '<path d="M832 288c0 53.020-42.98 96-96 96s-96-42.98-96-96 42.98-96 96-96 96 42.98 96 96z"></path>', '<path d="M896 832h-768v-128l224-384 256 320h64l224-192z"></path>'].join(''));

	        // pan glyph
	        _this.defs.append('symbol').attr('id', 'glyph-pan').attr('viewBox', '0 0 1024 1024').append('path').attr('d', 'M512 704l64-64v192h128l-192 192-192-192h128v-192zM512 321.984l-64 62.016v-192h-128l192-192 192 192h-128v192zM320 512l64 64h-192v128l-192-192 192-192v128h192zM702.016 512l-62.016-64h192v-128l192 192-192 192v-128h-192z');

	        // annotate glyph
	        _this.defs.append('symbol').attr('id', 'glyph-annotate').attr('viewBox', '0 0 1024 1024').append('path').attr('d', 'M864 0c88.364 0 160 71.634 160 160 0 36.020-11.91 69.258-32 96l-64 64-224-224 64-64c26.742-20.090 59.978-32 96-32zM64 736l-64 288 288-64 592-592-224-224-592 592zM715.578 363.578l-448 448-55.156-55.156 448-448 55.156 55.156z');
	        ...
	        var gly = _this.attr.svg.append("g").attr("transform", "translate(" + (_this.innerwidth + _this.attr.margin.left + 5) + "," + (_this.innerheight - _this.attr.margin.bottom - gdata.length * 22 + 52) + ")").selectAll(".glyphbox").data(gdata).enter().append("g").attr("class", "glyphbox").attr("id", function (d) {
	            return d;
	            ...
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.html(d).style("visibility", "visible").style("left", d3.event.clientX + 10 + "px").style("top", d3.event.clientY - 10 + "px");
	            }
	        }).on("mousemove", function (d) {
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.style("left", d3.event.clientX + 10 + "px").style("top", d3.event.clientY - 10 + "px");
	            }
	            ...
	            switch (d) {
	                ...
	                case 'zoom':
	                    ...
	                    d3.select(this).selectAll('.glyph').style('stroke-width', _this.enabled.zoom ? 2 : 1).style('stroke', _this.enabled.zoom ? 'black' : '#ddd');
	                    break;

	                case 'pan':
	                    _this.enabled.pan = !_this.enabled.pan;
	                    _this.enabled.zoom = !_this.enabled.zoom;
	                    d3.select(this).selectAll('.glyph').style('stroke-width', _this.enabled.pan ? 2 : 1).style('stroke', _this.enabled.pan ? 'black' : '#ddd');
	                    break;
	                ...
	                case 'annotate':
	                    ...
	                    d3.select(this).selectAll('.glyph').style('stroke-width', _this.enabled.annotate ? 2 : 1).style('stroke', _this.enabled.annotate ? 'black' : '#ddd');
	                    break;
	        ...
	        if (_this.attr.gshape === "square") {
	            gly.append("rect").attr("class", "glyph").attr("width", 22).attr("height", 22).attr("rx", 5).attr("ry", 5).style("fill", "transparent");
	        } else {
	            gly.append("circle").attr("class", "glyph").attr("cx", 11).attr("cy", 7).attr("r", 11).style("fill", "transparent");
	        }

	        // rendering icons
	        gly.append('svg').attr('width', 22).attr('height', 22).append("use").attr('x', _this.attr.gshape === 'square' ? 4 : 4).attr('y', _this.attr.gshape === 'square' ? 4 : 0).attr('height', 14).attr('width', 14).attr("xlink:href", function (d) {
	            switch (d) {
	                case 'zoom':
	                    return '#glyph-search';
	                case 'pan':
	    ...
	    this.enablezoom = function () {
	        ...
	        function withinBounds(motion) {
	            if (motion.x > _this.innerwidth || motion.x < 0 || motion.y > _this.innerheight || motion.y < 0) {
	                return false;
	            }
	            return true;
	        }
	        ...
	        var viewbox = _this.attr.svg.append('path').attr('class', 'viewbox').attr("transform", "translate(" + _this.attr.margin.left + "," + _this.attr.margin.top + ")");
	        ...
	        .on("dragstart", function (d) {
	            ...
	            if (_this.enabled.zoom) {
	                ...
	                if (Math.abs(_this.zoomdata.x - movement.x) > 10 && _this.zoomdata.x > 0) {
	                    ...
	                    xval = [xmap(_this.zoomdata.x), xmap(movement.x)].sort(function (a, b) {
	                        return a - b;
	                    });
	                ...
	                if (Math.abs(_this.zoomdata.y - movement.y) > 10 && _this.zoomdata.y < _this.innerheight) {
	                    ...
	                    yval = [ymap(_this.zoomdata.y), ymap(movement.y)].sort(function (a, b) {
	                        return a - b;
	                    });
	            ...
	            if (_this.enabled.zoom) {
	                ...
	                viewbox.attr('d', ['M' + _this.zoomdata.x + ',' + _this.zoomdata.y, 'L' + _this.zoomdata.x + ',' + movement.y, 'L' + movement.x + ',' + movement.y, 'L' + movement.x + ',' + _this.zoomdata.y, 'Z'].join(''));
	                ...
	                var xval = _.map(_this.xscale.range(), function (x) {
	                    return xmap(x - d3.event.dx);
	                });
	                var yval = _.map(_this.yscale.range(), function (x) {
	                    return ymap(x - d3.event.dy);
	                });
	        ...
	        .on("dblclick.zoom", null).on("mousedown.zoom", null).on("touchstart.zoom", null).on("touchmove.zoom", null).on("touchend.zoom", null);

	        // set up events for toggling enabled flags
	        quorra.events.Shift.down = function () {
	            d3.selectAll('.glyphbox#pan').selectAll('.glyph').style('stroke', 'black').style('stroke-width', 2);
	        ...
	        quorra.events.Shift.up = function () {
	            d3.selectAll('.glyphbox#pan').selectAll('.glyph').style('stroke', '#ddd').style('stroke-width', 1);
	            _.each(Object.keys(quorra.plots), function (key) {
	    ...
	    this.enableannotation = function () {
	        ...
	        _this.attr.svg.on('click', function () {
	            if (_this.enabled.annotate) {
	                ...
	                if (coordinates.x > _this.innerwidth + _this.attr.margin.left || coordinates.x < _this.attr.margin.left || coordinates.y > _this.innerheight + _this.attr.margin.top || coordinates.y < _this.attr.margin.top) {
	                    return;
	                }
	                ...
	                var obj = quorra.annotation(attr).bind(_this.go).x(d.x).y(d.y);
	                attr.x = d.x;
	                attr.y = d.y;
	        ...
	        quorra.events.ShiftA.down = function () {
	            d3.selectAll('.glyphbox#annotate').selectAll('.glyph').style('stroke', 'black').style('stroke-width', 2);
	            _.each(Object.keys(quorra.plots), function (key) {
	                quorra.plots[key].enabled.annotate = true;
	            });
	        };
	        quorra.events.ShiftA.up = function () {
	            d3.selectAll('.glyphbox#annotate').selectAll('.glyph').style('stroke', '#ddd').style('stroke-width', 1);
	            _.each(Object.keys(quorra.plots), function (key) {
	    ...
	    this.attr = extend({
	        ...
	        group: function group(d, i) {
	            return typeof d.group === 'undefined' ? 0 : d.group;
	        },
	        label: function label(d, i) {
	            return typeof d.label === 'undefined' ? i : d.label;
	        },
	    ...
	    _this.go.annotation = function (value) {
	        if (!arguments.length) return _.map(_this.attr.annotation, function (d) {
	            return d.__parent__.attr;
	        });
...
/***/ function(module, exports) {
	...
	var parameterize = function parameterize(attributes, generator) {
	    /**
	    parameterize()
	...
	var extend = function extend(stock, custom) {
	    /**
	    extend()
	    ...
	    */
	    ...
	    _.map(Object.keys(stock), function (d) {
	        if (typeof custom[d] !== 'undefined') {
	            if (_typeof(stock[d]) === "object" && !Array.isArray(stock[d]) && stock[d] != null) {
	...
	var selectmerge = function selectmerge(selection, entry, type) {
	    /**
	    selectmerge()
	...
	function Annotation(attributes) {
	    ...
	    */
	    ...
	    this.go = function () {
	        ...
	        if (_this.attr.tooltip == true) {
	            _this.attr.tooltip = d3.select("body").append("div").attr("class", "annotation-tooltip").attr("id", +_this.attr.id + "-tooltip").style("position", "fixed").style("visibility", "hidden").style("zindex", 999);
	        }
	        ...
	        var asel = _this.plot.plotarea.append('g').attr('id', _this.attr.id).attr('class', cl).style("visibility", function () {
	            return _.contains(_this.plot.attr.toggled, _this.plot.attr.group(_this.attr)) ? 'none' : 'visible';
	        }).style('opacity', _this.attr.style.opacity).on('mouseover', function () {
	            d3.select(this).style('opacity', _this.attr.hoveropacity);
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.html(_this.attr.hovertext).style("visibility", "visible").style("left", d3.event.pageX + 5 + "px").style("top", d3.event.pageY - 20 + "px");
	            }
	        }).on('mousemove', function () {
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.style("left", d3.event.pageX + 5 + "px").style("top", d3.event.pageY - 20 + "px");
	            }
	        ...
	        if (_this.attr.type === 'rect') {
	            aobj = asel.selectAll('.rect').data([_this.attr]).enter().append('rect').attr('class', 'rect').attr('transform', 'rotate(' + _this.attr.rotate + ' ' + x + ' ' + y + ')').attr('width', Math.abs(xscale(_this.attr.width) - xscale(0))).attr('height', Math.abs(yscale(_this.attr.height) - yscale(0))).attr('x', x).attr('y', y);
	        } else if (_this.attr.type === 'circle') {
	            aobj = asel.selectAll('.circle').data([_this.attr]).enter().append('circle').attr('class', 'circle').attr('r', _this.attr.size / 2).attr('cx', x).attr('cy', y);
	        } else if (_this.attr.type === 'triangle') {
	            aobj = asel.selectAll('.triangle').data([_this.attr]).enter().append('path').attr('class', 'triangle').attr('transform', 'rotate(' + _this.attr.rotate + ' ' + x + ' ' + y + ')').attr('d', function (d) {
	                return ['M' + (x - d.size / 2) + ',' + (y - d.size / 2), 'L' + (x + d.size / 2) + ',' + (y - d.size / 2), 'L' + x + ',' + (y + d.size / 2), 'Z'].join('');
	            });
	        }
	        ...
	        if (_this.attr.type !== 'text') {
	            _.map(_.without(Object.keys(_this.attr.style), 'opacity'), function (i) {
	                aobj.style(i, _this.attr.style[i]);
	            });
	        }

	        // show text
	        if (_this.attr.text !== '') {
	            asel.selectAll('.text').data([_this.attr]).enter().append('text').attr('class', 'text').attr('x', function (d) {
	            ...
	            }).style("font-size", _this.attr.tsize).style("text-anchor", "middle").text(_this.attr.text);
	        }
	    ...
	    */
	    ...
	    this.plot = function () {
	        ...
	        if (typeof _this.plotdata[0].x === 'string') {
	            _this.plotdata = _this.plotdata.sort(function (a, b) {
	                return a.x > b.x;
	            });
	        }
	        for (var grp in ugrps) {
	            var flt = _.filter(_this.plotdata, function (d) {
	                return d.group == ugrps[grp];
	            });
	            flt = _.map(flt, function (d) {
	                d.layer = grp;
	                return d;
	            });
	            layers.push(_.filter(flt, function (d) {
	                return d.group == ugrps[grp];
	            }));
	        }
	        var y0 = _.map(layers[0], function (d) {
	            return 0;
	        });
	        ...
	        // plotting bars
	        var layer = _this.plotarea.selectAll(".layer").remove().data(layers).enter().append("g").attr("class", "layer");

	        var bar = layer.selectAll("rect").remove().data(function (d) {
	            return d;
	        ...
	        .attr("y", function (d, i) {
	            ...
	            return _this.attr.layout == "stacked" ? _this.yscale(_this.ymapper(d.y0)) - _this.yscale(_this.ymapper(d.y0 + d.y)) : _.max([_this.innerheight - _this.yscale(_this.ymapper(d.y)), 0]);
	            ...
	            return _this.pallette(d.group);
	        }).style("opacity", _this.attr.opacity).style("visibility", function (d) {
	            return _.contains(_this.attr.toggled, d.group) ? 'hidden' : 'visible';
	        }).on("mouseover", function (d, i) {
	            d3.select(this).style("opacity", 0.25);
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.html(d.label).style("visibility", "visible").style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	            }
	        }).on("mousemove", function (d) {
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	            }
	        }).on("mouseout", function (d) {
	    ...
	    */
	    ...
	    this.plot = function () {
	        ...
	        var path = d3.svg.line().x(function (d, i) {
	            return _this.xscale(_this.xmapper(_this.attr.x(d, i)));
	        }).y(function (d, i) {
	            return _this.yscale(_this.ymapper(_this.attr.y(d, i)));
	        ...
	        for (var grp in ugrps) {
	            ...
	            // lines
	            var subdat = _.filter(_this.plotdata, function (d) {
	                return d.group == ugrps[grp];
	            });
	            if (subdat.length == 0) {
	                continue;
	            }
	            _this.plotarea.append("path").datum(subdat).attr("class", function (d, i) {
	                ...
	                if (_this.attr.layout === "line") {
	                    ...
	                    return [p, "L" + _this.xscale(_this.xmapper(_this.domain[_this.domain.length - 1])) + "," + _this.yscale(_this.ymapper(_this.range[0])), "L" + _this.xscale(_this.xmapper(_this.domain[0])) + "," + _this.yscale(_this.ymapper(_this.range[0])), "Z"].join('');
	                }
	            ...
	            }).style("stroke-width", _this.attr.size).style("opacity", _this.attr.opacity).style("visibility", function (d, i) {
	                ...
	                if (_this.attr.tooltip) {
	                    _this.attr.tooltip.html(d[0].group).style("visibility", "visible").style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	                }
	            }).on("mousemove", function (d) {
	                if (_this.attr.tooltip) {
	                    _this.attr.tooltip.style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	        ...
	        if (_this.attr.points > 0) {
	            ...
	            _this.plotarea.selectAll(".dot").remove().data(_this.plotdata).enter().append("circle").attr("class", function (d, i) {
	                return "dot " + "g_" + d.group;
	            }).attr("r", _this.attr.points).attr("cx", function (d, i) {
	                return _this.xscale(_this.xmapper(_this.attr.x(d, i)));
	            ...
	            }).style("opacity", _this.attr.opacity).style("visibility", function (d, i) {
	                return _.contains(_this.attr.toggled, _this.attr.group(d, i)) ? 'hidden' : 'visible';
	            }).on("mouseover", function (d, i) {
	                d3.select(this).style("opacity", 0.25);
	                if (_this.attr.tooltip) {
	                    _this.attr.tooltip.html(_this.attr.label(d, i)).style("visibility", "visible").style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	                }
	            }).on("mousemove", function (d) {
	                if (_this.attr.tooltip) {
	                    _this.attr.tooltip.style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	...
	function Histogram(attributes) {
	    ...
	    */
	    ...
	    _this.attr.transform = function (data) {
	        ...
	        var histogram = d3.layout.histogram().frequency(false).bins(d3.scale.linear().ticks(_this.attr.bins));
	        ...
	        for (var grp in grps) {
	            ...
	            var newgrp = histogram(_.map(subdat, function (d) {
	                return d.x;
	            }));
	            newgrp = _.map(newgrp, function (d) {
	...
	function Multiline(attributes) {
	    ...
	    */
	    ...
	    this.axes = function () {
	        ...
	        // x axis
	        _this.plotregion.append("g").attr("class", "x axis").call(_this.xaxis);

	        _this.plotregion.selectAll(".x.axis .domain").remove();

	        // y scale formatting
	        _this.yscale = d3.scale.linear().domain([0, 1]).range([_this.innerheight, 0]);
	        _this.yscales = [];
	        _this.yaxes = [];
	        var xdiff = _this.xscale(1) - _this.xscale(0);
	        for (var i = 0; i < _this.domain.length; i++) {
	            var xdat = _.filter(_this.data, function (d) {
	                return d.x === _this.domain[i];
	            });
	            xdat = _.map(xdat, function (d) {
	                return d.y;
	            });
	            ...
	            var scale = d3.scale.linear().domain(domain).range([0, 1]);
	            _this.yscales.push(scale);
	            var vscale = d3.scale.linear().domain(domain).range(_this.yscale.range()).nice();
	            ...
	            _this.plotregion.append("g").attr("transform", "translate(" + _this.xscale(i) + ",0)").attr("class", "y axis").call(axis);
	        }
	    };
	        ...
	        for (var grp in ugrps) {
	            ...
	            _this.plotarea.append("path").datum(subdat).attr("class", function (d, i) {
	                ...
	                if (_this.attr.layout === "line") {
	                    ...
	                    return [p, "L" + _this.xscale(_this.xmapper(_this.domain[_this.domain.length - 1])) + "," + _this.yscale(0), "L" + _this.xscale(_this.xmapper(_this.domain[0])) + "," + _this.yscale(1), "Z"].join('');
	                }
	            }).style("fill", function (d) {
	                if (_this.attr.layout === "line") {
	                    return "none";
	...
	function Pie(attributes) {
	    ...
	    */
	    ...
	    this.plot = function () {
	        ...
	        for (var i in gps) {
	            var subdat = _.filter(_this.data, function (d) {
	                return d.group == gps[i];
	            });
	            newdata.push({
	                x: _this.attr.aggregate(_.map(subdat, function (d) {
	                    return d.x;
	                })),
	                group: gps[i],
	        ...
	        // plot
	        var g = _this.plotregion.attr("transform", "translate(" + _this.innerwidth / 2 + "," + _this.innerheight / 2 + ")").selectAll(".arc").data(pie(newdata)).enter().append("g").attr("class", function (d) {
	        ...
	        g.append("path").attr("d", arc).style("fill", function (d, i) {
	            return _this.pallette(_this.attr.group(d.data, i));
	        }).style("opacity", _this.attr.opacity).on("mouseover", function (d, i) {
	            d3.select(this).style("opacity", 0.25);
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.html(_this.attr.label(d.data, i)).style("visibility", "visible").style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	            }
	        }).on("mousemove", function (d) {
	            if (_this.attr.tooltip) {
	                _this.attr.tooltip.style("left", d3.event.clientX + 5 + "px").style("top", d3.event.clientY - 20 + "px");
	    ...
	    this.plot = function () {
	        ...
	        if (_this.attr.line !== false) {
	            ...
	            for (var grp in ugrps) {
	                ...
	                }).style("stroke-width", _this.attr.linesize).style("opacity", _this.attr.opacity).style("visibility", function (d, i) {
	                    if (_.contains(_this.attr.selected, _this.attr.group(d[0], i))) {
	                        return 'visible';
	                    }
	        ...
	        _this.plotarea.selectAll(".dot").remove().data(_this.plotdata).enter().append("circle").attr("class", function (d, i) {
	            return "dot " + "g_" + d.group;
	        }).attr("r", _this.attr.size).attr("cx", function (d, i) {
	            return d.x;
	        }).attr("cy", function (d, i) {
	            return d.y;
	        }).style("fill", function (d, i) {
	        ...
	        }).attr("clip-path", "url(#clip)").on("mouseover", function (d, i) {
	            if (_.contains(_this.attr.selected, _this.attr.group(d, i))) {
	                return;
	            }
	            if (_this.attr.line === 'hover') {
	        ...
	        if (_this.attr.xdensity) {
	            _this.plotarea.selectAll(".xtick").remove().data(_this.plotdata).enter().append("line").attr("clip-path", "url(#clip)").attr("class", function (d, i) {
	        ...
	        if (_this.attr.ydensity) {
	            _this.plotarea.selectAll(".ytick").remove().data(_this.plotdata).enter().append("line").attr("clip-path", "url(#clip)").attr("class", function (d, i) {
